      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {certifications.map((cert, idx) => (
          <BlurFade key={cert.title} delay={0.04 * (17 + idx)} className="h-full">
            <div
              className={`group relative flex flex-col justify-between p-6 border rounded-2xl bg-card backdrop-blur-sm shadow-sm transition-all duration-300 h-full overflow-hidden ${cert.hoverBorder} hover:-translate-y-1 hover:shadow-lg`}
            >
              ...
              <div className="z-10 flex items-center justify-between mt-auto pt-5 border-t border-border/50">
                ...
                <Link
                  href={cert.link}
                  target={cert.isInternal ? undefined : "_blank"}
                  rel={cert.isInternal ? undefined : "noopener noreferrer"}
